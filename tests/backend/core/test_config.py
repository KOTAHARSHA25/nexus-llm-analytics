...
from backend.core.config import Settings, get_settings, apply_environment_preset, validate_config
...
def test_validators():
    with patch.dict(os.environ, {}, clear=True):
        # Test CORS validator
        s = Settings(_env_file='non_existent', CORS_ALLOWED_ORIGINS="a.com, b.com")
        assert "a.com" in s.cors_allowed_origins
        assert "b.com" in s.cors_allowed_origins
        
        # Test file extensions
        s = Settings(_env_file='non_existent', ALLOWED_FILE_EXTENSIONS="TXT, PDF ")
        assert "txt" in s.allowed_file_extensions
        assert "pdf" in s.allowed_file_extensions
        
        # Test log level
        s = Settings(_env_file='non_existent', LOG_LEVEL="debug")
        assert s.log_level == "DEBUG"
        
        with pytest.raises(ValueError):
            Settings(_env_file='non_existent', LOG_LEVEL="INVALID")
