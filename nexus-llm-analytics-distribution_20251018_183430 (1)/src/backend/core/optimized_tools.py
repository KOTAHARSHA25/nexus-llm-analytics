...
class OptimizedRAGTool(BaseTool):
    ...
    def _generate_optimized_summary(self, ranked_docs: List[Dict], query: str) -> str:
        ...
        max_context_length = 2000  # Optimal context size for performance
        context_parts = []
        current_length = 0
        ...
        for doc in ranked_docs:
            ...
            if current_length + len(content) <= max_context_length:
                context_parts.append(f"[Score: {doc['relevance_score']:.2f}] {content}")
                ...
                if remaining_space > 100:  # Only add if meaningful space left
                    ...
                    context_parts.append(f"[Score: {doc['relevance_score']:.2f}] {truncated_content}")
                break
        ...
        # Generate summary with adaptive timeout
        try:
            summary_response = self.llm_client.generate(
                prompt=f"""Based on the following relevant documents, provide a comprehensive answer to the query: "{query}"
...
2. Provide specific details and examples where available
3. If documents contain contradictory information, note the discrepancies
...
Answer:""",
            ...
            if "error" in summary_response:
                return f"Summary generation failed: {summary_response['error']}"
            
            response_text = summary_response.get('response', 'No response generated')
            
            # Add source information
            source_info = f"\n\nSources: {len(ranked_docs)} documents analyzed"
            if ranked_docs:
                top_source = ranked_docs[0].get('metadata', {})
                if top_source.get('filename'):
                    source_info += f" (Primary: {top_source['filename']})"
