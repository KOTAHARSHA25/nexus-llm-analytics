...
class MLInsightsAgent(BasePluginAgent):
    ...
    def initialize(self, **kwargs) -> bool:
        ...
        try:
            ...
            logging.info(f"✅ ML Insights Agent initialized")
            logging.info(f"   Random state: {self.random_state}")
            logging.info(f"   Default clusters: {self.n_clusters_default}")
            logging.info(f"   Available algorithms: {len(sum(self.available_algorithms.values(), []))}")
