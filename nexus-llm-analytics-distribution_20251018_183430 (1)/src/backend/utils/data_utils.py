...
class DataPathResolver:
    ...
    def ensure_directories_exist(cls):
        ...
        logger.info(f"Data directories ready: {uploads_dir}, {samples_dir}")
...
def read_dataframe(file_location: str, encoding: str = 'utf-8', sample_size: int = 4500) -> pd.DataFrame:
    ...
    try:
        ...
        logger.info(f"Successfully read {len(df)} rows from {file_location}")
    ...
    if cleaned_df.columns.tolist() != df.columns.tolist():
        logger.info(f"Cleaned column names in {file_location}")

    # Sample down if necessary
    if len(cleaned_df) > sample_size:
        logger.info(f"Dataframe has {len(cleaned_df)} rows. Sampling {sample_size} rows.")
    ...
    if cleaned_df.columns.tolist() != df.columns.tolist():
        ...
        if file_extension in write_funcs:
            try:
                ...
                logger.info(f"Saved cleaned dataframe to {file_location}")
            except Exception as e:
